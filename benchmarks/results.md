...
Build the core `typeset` Python API:
- `typeset.render(template_path, data_dict) -> bytes` (the main entry point)
- Jinja2 auto-escaping built in
- Intermediate `.typ` file preserved on error for debugging
- CLI command: `typeset render template.j2.typ data.json -o output.pdf`
